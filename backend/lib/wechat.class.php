 ...
 *   $weObj->valid();
 *   $type = $weObj->getRev()->getRevType();    76
 *            exit;
 *            break;
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '')
    {
        ...
        if ($result) {
            ...
            file_put_contents("test", "token = " . $json['access_token']);
    ...
    /**
     * 批量获取关注用户列表
     * @param string|unknown $next_openid
     * @return bool|mixed
     */
     ...
     * @return bool|string
     * @internal param string $cardid 卡券ID 必须
     */
    ...
    /**
     * 批量查询卡列表
     * @param int|开始拉取的偏移 $offset 开始拉取的偏移，默认为0从头开始
     * @param int|需要查询的卡片的数量 $count 需要查询的卡片的数量（数量最大50,默认50）
     * @return array|bool {
     * {
     * "errcode":0,
     * "errmsg":"ok",
     ...
     * @param int|number $bind 关联操作标志位， 0 为解除关联关系，1 为建立关联关系
     * @param int|number $append 新增操作标志位， 0 为覆盖，1 为新增
     * @param string $uuid UUID、major、minor，三个信息需填写完整，若填了设备编号，则可不填此信息
     * @param int $major
     * @param int $minor
     * @return bool|mixed 正确返回JSON 数据示例:
     * 正确返回JSON 数据示例:
     * {
     * "data": {
     * },
